...
description: "A Figma Variables export in JSON format generated by a plugin. This file represents raw plugin output and requires normalization before use with Variable Design Standard (VDS) tooling."
---
...
- Raw file: [figma-export.json](/assets/schema/figma-export.json)

**Note**: This is a **plugin-generated format**, not the native Figma REST API format. The REST API uses a different structure with `valuesByMode` and variable IDs. See [REST API format](#rest-api-format) for details.
...
- Uses `valuesByMode` object with mode IDs as keys
- Variables referenced by internal IDs (`VariableID:xxx:xxx`)
- Collections referenced by internal IDs (`VariableCollectionId:xxx:xxx`)
